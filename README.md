...
[FPGAOL](http://202.38.79.134)是一个线上硬件实验平台，通过将FPGA部署在云上，实现硬件资源的高效利用，并力图为学生提供与实际的FPGA相同甚至更好的使用体验。

FPGAOL v1.1是FPGAOL的最近一次更新。在本次更新中，我们增加了一些管脚并重点优化了数码管显示和波形图生成的功能。本说明介绍了该版本的使用方法，并提供了一些示例程序。为避免不必要的麻烦，我们**强烈建议**在实际编程中使用示例中提供的`fpgaol.xdc`文件。

## 获取FPGA

本版本中，我们引入了更完善的FPGA访问控制机制，用户可以在登陆后点击`acquire`按钮，并在后续给出的链接中访问获取到的FPGA。目前用户每次使用FPGA资源的时间默认为**10分钟**，超时后即被释放，用户也可以通过首页的`release`按钮主动释放FPGA。
...
除此之外，数码管的使用方式与现实中无异。和现实中的数码管一样，我们建议的扫描频率为**50Hz**，也就是说，若要驱动8个数码管，需要一个400Hz的时钟。
